...
with st.expander("❓ 자주 묻는 질문 (FAQ)"):
    st.markdown("""
    **Q: 500개 이상의 논문을 어떻게 한 번에 처리하나요?**
    A: WOS에서 1-500, 501-1000, 1001-1500... 순서로 여러 번 Plain Text 다운로드한 후, 모든 .txt 파일을 한 번에 업로드하면 자동으로 병합됩니다.
    ...
    **Q: 병합된 파일이 SCIMAT에서 제대로 로딩되지 않습니다.**
    A: 테스트 파일(100개)을 먼저 확인해보세요. 문제가 지속되면 원본 WOS 파일들의 형식을 다시 확인해주세요.
