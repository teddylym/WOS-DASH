with col1:
    ...
    success_content = "".join([f"<p style='color: #4a90e2; margin: 4px 0;'>{info}</p>" for info in success_info])
    
    st.markdown(f"""
    <div class="info-panel">
        <h4 style="color: #4a90e2; margin-bottom: 12px;">🏆 병합 성과</h4>
        {success_content}
    </div>
    """, unsafe_allow_html=True)
...
st.markdown("<br><br>", unsafe_allow_html=True)    full_text = ' '.join([title, source_title, author_keywords, keywords_plus, abstract])
...
# --- 메인 헤더 (첨부파일 스타일 적용, 내용은 기존 유지) ---
st.markdown("""
<div class="main-header-card">
    <div class="header-icon">📊</div>
    <div class="header-greeting">업테경님, 안녕하세요.</div>
    <div class="header-subtitle">WOS PREP - SCIMAT Edition</div>
    <div class="header-link">보유 카드 보기 ></div>
</div>
""", unsafe_allow_html=True)

# --- 핵심 기능 소개 (첨부파일 3개 그리드 스타일 적용) ---
st.markdown("""
...
if uploaded_files:
    ...
    <div class="metric-grid">
        <div class="metric-card">
            <div class="metric-card-title">이용가능금액</div>
            <div class="metric-item">
                <div class="metric-label">일시불/할부</div>
                <div class="metric-value large">{total_papers:,}편</div>
            </div>
            <div class="metric-item">
                <div class="metric-label">단기카드대출 (현금서비스)</div>
                <div class="metric-value">{include_papers:,}편</div>
            </div>
            <div class="metric-item">
                <div class="metric-label">장기카드대출 (카드론)</div>
                <div class="metric-value">{len(merged_df[merged_df['Classification'].str.contains('Review', na=False)]):,}편</div>
            </div>
        </div>
        <div class="metric-card">
            <div class="metric-card-title">결제예정금액</div>
            <div class="metric-item">
                <div class="metric-label">이번<br>오늘 입금 시</div>
                <div class="metric-value large">{successful_files:,}개</div>
            </div>
            <div class="metric-item">
                <div class="metric-label">다음<br>10월 10일</div>
                <div class="metric-value">{duplicates_removed:,}편</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div class="metric-card">
            <div class="metric-icon">📋</div>
            <div class="metric-value">{len(df_final_output):,}</div>
            <div class="metric-label">최종 분석 대상<br><small style="color: #6c757d;">(Exclude 제외import streamlit as st
import pandas as pd
...
# --- 첨부파일 스타일만 적용한 CSS (기능과 텍스트는 기존 유지) ---
st.markdown("""
<style>
...
def classify_article(row):
    ...
    full_text = ' '.join([title, source_title, author_keywords
