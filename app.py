...
import io
import base64 # 로고 삽입을 위해 추가

# 페이지 설정 (원본 유지)
st.set_page_config(page_title="WOS 전처리", layout="wide", initial_sidebar_state="expanded")

# --- 로고 파일 Base64 인코딩 함수 (로고 표시를 위해 추가된 부분) ---
def get_image_as_base64(path):
    try:
        with open(path, "rb") as image_file:
            return base64.b64encode(image_file.read()).decode()
    except FileNotFoundError:
        # 파일이 없을 경우, None을 반환하여 오류 대신 경고 메시지를 표시하도록 함
        return None

# --- 커스텀 CSS (색상만 수정한 원본 코드) ---
st.markdown("""
<style>
.css-1v3fvcr {
...
.st-br {
    background: #003C71; /* 요청하신 대로 색상 수정: #0096FF -> #003C71 */
}
</style>
...
<div style="display: flex; align-items: center; margin-bottom: 20px;">
    {'<img src="data:image/png;base64,{}" style="height: 50px; margin-right: 20px;">'.format(encoded_logo) if encoded_logo else ''}
    <h1 style="margin: 0;">WOS 전처리 및 키워드 정규화</h1>
</div>
""", unsafe_allow_html=True)

# 로고 파일이 없을 경우 경고 메시지 표시
if not encoded_logo:
    st.warning(f"로고 파일을 찾을 수 없습니다. `{LOGO_PATH}` 파일이 코드와 동일한 폴더에 있는지 확인해주세요.")
